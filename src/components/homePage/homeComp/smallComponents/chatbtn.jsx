import { useState } from "react";
import "./../styles/chatbtn.css"
import CloseIcon from '@mui/icons-material/Close';
import BoltIcon from '@mui/icons-material/Bolt';

const pillUpLogo = "data:image/jpeg;base64,"

function Chatbtn(){

    const customStyle = {
        visibility:"hidden"
    }
    const [isShowingChat , setShowChat] = useState(false)

    function handleClick(){
        setShowChat(!isShowingChat);
    }

    return(
        <div className="chat-container">
            <div id={isShowingChat ? "chat-ui" : "chat-ui-closed"} style={isShowingChat ? null: customStyle}>
                <div className="chat-heading">
                    <img height="68px"  src={pillUpLogo} alt="pillup logo" />
                    <h3>PillUp</h3>
                    <p>Always there for your medicine needs</p>
                    <button onClick={handleClick}><CloseIcon /></button>
                </div>

                <div className="chat-main">
                    <div className="chat-main-div">
                       <p>PillUp</p>
                       <p>Hello! Greetings from PillUp. How can we help you today ?</p>
                    </div>
                </div>
                <div className="chat-footer">
                    <a  href="https://api.whatsapp.com/send/?phone=%2B918920878094&text=Hi+I+am+interested+in+ordering+from+PillUp&type=phone_number&app_absent=0" target="_blank" rel="noreferrer" >Start chat</a>
                    <p style={{margin:"0px 0px 0px 20px"}}><BoltIcon sx={{color:"rgb(255, 238, 0)", fontSize:"22px", left:"134px ", position:"absolute"}} /> by <i style={{fontWeight:"600", color:"#08cf65"}}> AiSensy</i></p>
                </div>
            </div>

            <button id="whatapp-logo" className={isShowingChat ? "whatsapp-btn-closed" : null} onClick={handleClick}>
                <img src="https://cdn-icons-png.freepik.com/256/15707/15707917.png?semt=ais_hybrid" alt="whatsapp logo"/>
                {!isShowingChat && "Chat with us"} 
            </button>
        </div>
    )
}

export default Chatbtn;